...
- **wife-rating/** — Vanilla JS "wife performance review" feedback app (a joke, but not really). Color-coded star ratings across marriage categories, highlight chips, comments; results are base64-encoded into a shareable link (no backend). Entry: `wife-rating/index.html`
...
- **Run agent team bot:** `cd agent-team && python main.py` (needs `TELEGRAM_BOT_TOKEN` + `ANTHROPIC_API_KEY`)
